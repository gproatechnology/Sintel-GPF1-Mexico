...
import { useDashboardUpdates } from '@/services/websocket'
import { DashboardSettings, useWidgetVisibility } from '@/components/DashboardSettings'
...
export default function Dashboard() {
  ...
  const { getWidgetVisibility, getRefreshInterval } = useWidgetVisibility()
  ...
  const handleNewConsumption = (data) => {
    console.log('[Dashboard] New consumption received:', data)
    // Refresh dashboard data when new consumption is registered
    queryClient.invalidateQueries({ queryKey: ['dashboardStats'] })
    queryClient.invalidateQueries({ queryKey: ['dailySummary'] })
    toast.success('Nuevo consumo registrado', {
      icon: '🍽️',
      duration: 2000
    })
  }
  ...
  useDashboardUpdates(handleNewConsumption)
  ...
  const { data: stats } = useQuery({
    ...
    refetchInterval: USE_MOCK_DATA ? false : (getRefreshInterval() * 1000),
  })
